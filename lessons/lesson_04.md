...
> **ЗАДАНИЕ**. Определить модуль `User` в файле [user.ex](../lib/user.ex). Модуль должен содержать аттрибут `greeting` в котором установлено значение "hello".
> В модуле требуется определить публичный метод `welcome/1` который принимает на вход параметр `name` и конкатинирует аттрибут `greeting` с переданным параметром `name`, добавив пробел между значениями.

> **Подсказка** Запустить команду `make format` для форматирования стиля. 
>
> **Самопроверка**. Запустить команду `make verify` для проверки кода.
> Запустить тест: `mix test test/user_test.exs`