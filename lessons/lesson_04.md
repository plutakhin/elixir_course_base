...
> В модуле требуется определить публичный метод `welcome/1` который принимает на вход параметр `name` и конкатинирует аттрибут с переданным параметром, добавив пробел между значениями.
